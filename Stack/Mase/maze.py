    ...
    def reset( self ):
        for cur_row in range(self.num_rows()):
            for cur_col in range(self.num_cols()):
                if self._mazeCells[cur_row, cur_col] == self.TRIED_TOKEN or self._mazeCells[cur_row, cur_col] == self.PATH_TOKEN:
                    self._mazeCells[cur_row, cur_col] = None

    # Prints a text-based representation of the maze.
    def draw( self ):
        for cur_row in range(self.num_rows()):
            for cur_col in range(self.num_cols()):
                if self._mazeCells[cur_row, cur_col] == None:
                    draw_line = ' '
                else:
                    draw_line = self._mazeCells[cur_row, cur_col]
                print(draw_line, end='')
